...
- [项目实现路径.md](项目实现路径.md)：较完整的参赛实现方案。
- [参赛路径规划.md](参赛路径规划.md)：赛道选择和技术方向分析。
- `scripts/verify_oulad.py`：OULAD 数据可行性验证脚本。
...
本地验证需要把 OULAD 文件放入 `oulad_raw/`，然后运行：

```bash
python3 scripts/verify_oulad.py
```
